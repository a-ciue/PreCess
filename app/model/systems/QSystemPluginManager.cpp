...
bool QSystemPluginManager::registerPlugin(const QString& plugin_path)
{
    std::filesystem::path path_std(plugin_path.toStdWString()); 
    bool result = system_plugin_manager_->registerPlugin(path_std);
    if (result) {
        plugin_paths_.append(plugin_path);
        plugin_names_.append(QFileInfo(plugin_path).fileName());
