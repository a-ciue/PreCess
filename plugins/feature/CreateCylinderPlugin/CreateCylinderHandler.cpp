...
std::any CreateCylinderHandler::execute(FeatureContext& ctx)
{
    ...
    try {
        ...
        if (target_model_id < 0)
            target_model_id = ctx.model.addModel("temp_Cylinder_" + std::to_string(next_cylinder_number_), {});
        auto geometry = std::make_unique<GeometryData>();
        geometry->setRootShape(std::move(shape));
        auto component = std::make_unique<ComponentData>();
        component->name = "Cylinder_" + std::to_string(next_cylinder_number_);
        ...
        ++next_cylinder_number_; // 新建组件，推进编号
        return component_id;
