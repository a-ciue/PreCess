...
std::any CreatePointHandler::execute(FeatureContext& ctx)
{
    ...
    try {
        ...
        if (target_model_id < 0)
            target_model_id = ctx.model.addModel("temp_Point_" + std::to_string(next_point_number_), {});
        auto geometry = std::make_unique<GeometryData>();
        geometry->setRootShape(std::move(shape));
        auto component = std::make_unique<ComponentData>();
        component->name = "Point_" + std::to_string(next_point_number_);
