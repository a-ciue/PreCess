...
static Mesh_meshIO read_buffer(std::istream& in, const fs::path& current_file)
{
    ...
    while (!line.empty()) {
        ...
        if (key == "NODE") {
            ...
            while ((int)(token_buf.size() - token_offset) >= num_data) {
                std::vector<int> nodes;
                nodes.reserve(nodes_per_cell);
                int orig_eid = token_buf[token_offset];
                for (int k = 1; k <= nodes_per_cell; ++k)
                    nodes.push_back(token_buf[token_offset + k]);
                token_offset += num_data;
            ...
            if (token_offset > 0) {
                token_buf.erase(token_buf.begin(), token_buf.begin() + static_cast<long long>(token_offset));
            }
...
void write_abaqus(const std::string& filename, const Mesh_meshIO& mesh, const std::string& float_fmt, bool translate_cell_names)
{
    ...
    if (float_fmt.find("%s") != std::string::npos ||
        float_fmt.find("%n") != std::string::npos ||
        float_fmt.find("%p") != std::string::npos ||
        float_fmt.find("%x") != std::string::npos ||
        float_fmt.find("%X") != std::string::npos) {
        throw std::runtime_error("Invalid float format string: dangerous format specifier");
    }
