...
struct PlyData {
    ...
    size_t count { 0 };
    bool isList { false };
    std::vector<size_t> listSizes; // per-item list sizes when property is a list (populated during read)
};
        ...
        for (size_t i = 0; i < e.size; ++i) {
            ...
            for (auto& p : e.properties) {
                ...
                if (p.isList) {
                    if (0==p.listCount) {
                        // �䳤�б����������ж�ȡ�б�����
                        uint8_t listSize = *(helper->data->buffer.get() + helper->cursor->byteOffset);
                        helper->cursor->byteOffset += sizeof(uint32_t);
                        os << static_cast<int>(listSize) << " ";
            ...
            for (auto& property : element.properties) {
                ...
                if (!lookup.skip) {
                    ...
                    if (firstPass) {
                        ...
                        // 如果这是一个 list 属性，在 firstPass 时记录每个条目的 listSize，
                        // 以便分配缓冲区后，调用方可以获得每个元素的实际长度（支持混合长度）。
                        if (property.isList) {
                            helper->data->listSizes.push_back(listSize);
                        }
                    } else {
