...
ModelActor::ModelActor(const Handle(TDocStd_Document)& doc) : doc_(doc) {
...
ModelActor::~ModelActor() {
    if (face_renderer_) {
        for (const auto& [label, actor] : patch_actors_) {
            face_renderer_->RemoveActor(actor);
        }
    }
    if (block_renderer_) {
        for (const auto& [label, actor] : block_actors_) {
            block_renderer_->RemoveActor(actor);
        }
    }
    if (group_renderer_) {
        for (const auto& [label, actor] : group_actors_) {
...
void ModelActor::bind_renderer(vtkRenderer* renderer, RenderMode mode) {
    ...
    for (auto&& [label, actor] : *mode_actors) {
...
void ModelActor::merge_blocks(const TDF_LabelSequence& block_ids, TDF_Label father_block, const TDF_LabelSequence& father_block_patches) {
    for (Standard_Integer i = 1; i <= block_ids.Length(); ++i) {  // 从 1 开始遍历 TDF_LabelSequence
        const TDF_Label& label = block_ids.Value(i);
        if (!label.IsEqual(father_block)) {  // 使用 IsEqual 比较 TDF_Label
            vtkActor* erase_actor = block_actors_[label];
            if (block_renderer_) {
                block_renderer_->RemoveActor(erase_actor);  // 从 renderer 中移除 actor
            }
            block_actors_.erase(label);  // 从映射中删除 actor
        }
    }

    // 更新留下的 block
    update_block(father_block);
}

void ModelActor::merge_groups(const TDF_LabelSequence& group_ids, TDF_Label father_group, const TDF_LabelSequence& father_group_blocks) {
    for (Standard_Integer i = 1; i <= group_ids.Length(); ++i) {  // 从 1 开始遍历 TDF_LabelSequence
        const TDF_Label& label = group_ids.Value(i);
        if (!label.IsEqual(father_group)) {  // 使用 IsEqual 比较 TDF_Label
            vtkActor* erase_actor = group_actors_[label];
            if (group_renderer_) {
                group_renderer_->RemoveActor(erase_actor);  // 从 renderer 中移除 actor
            }
            group_actors_.erase(label);  // 从映射中删除 actor
        }
    }

    // 更新留下的 group
    update_group(father_group);
}

void ModelActor::update_patch(const TDF_Label& patch_label, const std::vector<std::array<double, 3>>& points, const std::vector<std::array<int, 3>>& triangles) {
    vtkSmartPointer<vtkActor>& patch_actor = patch_actors_[patch_label];
    if (!patch_actor) {
        patch_actor = vtkSmartPointer<vtkActor>::New();
    }
    ...
    patch_actor->GetProperty()->SetDiffuseColor(0.8, 0.8, 0.8);
...
void ModelActor::update_block(const TDF_Label& block_label) {
    if (block_actors_.find(block_label) == block_actors_.end()) {
        block_actors_[block_label] = vtkSmartPointer<vtkActor>::New();  // 创建新的 vtkActor
    }

    vtkSmartPointer<vtkActor> block_actor = block_actors_[block_label];

    // 遍历子标签获取 patch_labels
    std::vector<TDF_Label> patch_labels;
    for (TDF_ChildIterator it(block_label); it.More(); it.Next()) {
        TDF_Label patch_label = it.Value();
        patch_labels.push_back(patch_label);
    }

    // 合并 patch 的 vtkActor
    std::vector<vtkActor*> patch_actors;
    for (const TDF_Label& patch_label : patch_labels) {
        if (patch_actors_.find(patch_label) != patch_actors_.end()) {
            patch_actors.push_back(patch_actors_[patch_label]);
        }
    }

    _merge_actors(block_actor, patch_actors);
}

void ModelActor::update_group(const TDF_Label& group_label) {
    if (group_actors_.find(group_label) == group_actors_.end()) {
        group_actors_[group_label] = vtkSmartPointer<vtkActor>::New();  // 创建新的 vtkActor
    }

    vtkSmartPointer<vtkActor> group_actor = group_actors_[group_label];

    // 遍历子标签获取 block_labels
    std::vector<TDF_Label> block_labels;
    for (TDF_ChildIterator it(group_label); it.More(); it.Next()) {
        TDF_Label block_label = it.Value();
        block_labels.push_back(block_label);
    }

    // 合并 block 的 vtkActor
    std::vector<vtkActor*> block_actors;
    for (const TDF_Label& block_label : block_labels) {
        if (block_actors_.find(block_label) != block_actors_.end()) {
            block_actors.push_back(block_actors_[block_label]);
        }
    }

    _merge_actors(group_actor, block_actors);
}


int ModelActor::block_actor_id(vtkActor* actor) {
    for (const auto& [block_label, block_actor] : block_actors_) {
        if (block_actor == actor) {
            return block_label.Tag();  // 返回 block_label 的唯一 ID
        }
    }
    throw std::runtime_error("block actor not found");
}
int ModelActor::group_actor_id(vtkActor* actor) {
    for (const auto& [group_label, group_actor] : group_actors_) {
        if (group_actor == actor) {
            return group_label.Tag();  // 返回 group_label 的唯一 ID
        }
    }
    throw std::runtime_error("group actor not found");
}
int ModelActor::patch_actor_id(vtkActor* actor) {
    for (const auto& [patch_label, patch_actor] : patch_actors_) {
        if (patch_actor == actor) {
            return patch_label.Tag();  // 返回 patch_label 的唯一 ID
        }
    }
    throw std::runtime_error("patch actor not found");
}

void ModelActor::_merge_actors(vtkActor* father_actor, const std::vector<vtkActor*>& actors) {
    if (!father_actor) {
        throw std::invalid_argument("Father actor cannot be null.");
    }

    // 合并子 actors 的 PolyData
    ...
    double g = ModelUtil::randomSequence->GetNextRangeValue(0.1, 0.6);
    double b = ModelUtil::randomSequence->GetNextRangeValue(0.1, 0.6);
    ...
    father_actor->GetProperty()->SetSpecularColor(ModelUtil::colors->GetColor3d("White").GetData());
