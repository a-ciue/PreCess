 ...
 * - 功能在 **GUI 线程** 的 setup() 中经 InteractionContext 注册回调与初始状态；
...
struct InteractionState {
    ...
    //! @brief GUI 线程延迟操作：渲染线程消费执行后拉取标注刷新
    //!（覆盖语义：重复置位仅保留最新，操作须幂等；会话下线迁移时先消费再清理——
    //! 功能在 feature 级 deactivate() 中经 deferRefresh 挂的现场清理由此保证执行）
