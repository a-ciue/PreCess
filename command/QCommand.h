...
/**
 * @brief QCommand 类封装了一条可执行命令的元信息
 *
...
public:
     ...
     * @param argTypes 命令参数类型描述模型（可选），用于描述该命令所需参数
     * @param parent 父对象，默认为 nullptr
     */
    QCommand(const QString& name, CommandFactory factory, QStandardItemModel* argTypes = nullptr, QObject* parent = nullptr)
            : QObject(parent), name_(name), factory_(std::move(factory)), arg_types_(argTypes ? argTypes : new QStandardItemModel(this)) {}
     ...
     */
    Q_INVOKABLE QStandardItemModel* argTypes() const { return arg_types_; }
...
private:
    ...
    QStandardItemModel* arg_types_;                //!< 命令参数类型描述模型指针
};
