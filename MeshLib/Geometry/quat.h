...
*      \file Quat.h
*      \brief Quaternion
*	   \author David Gu  //也就是顾老师！
*      \date 10/07/2010  //久远
*
*/

#ifndef _QUAT_H_  //这两行默认加上，还有结尾处的endif，注意命名
#define _QUAT_H_

#include <math.h>
#include "Point.h"

typedef double mat44[4][4];     //这里的语法不是很明白

namespace MeshLib  //这里的语法不是很明白
{
...
public:
  ...
  */
  ...
  CQrot( const CQrot & q )   //常引用保护了参数的值，使其不会被改变
  {
    m_w =  q.m_w;
    m_x =  q.m_x;
    m_y =  q.m_y;
  ...
  */
  ...
  /*operator 是C++的一个关键字，它和运算符（如 = ）一起使用，表示一个运算符重载函数
	...
	CQrot & operator=(const CQrot & q) //作用是什么？
  {
      m_w = q.m_w;
      m_x  = q.m_x;
      m_y  = q.m_y;
